...
const TimeEnd: React.FC = () => {
  ...
  const handleDateUpdate = async () => {
    try {
      const endDocRef = doc(db, "settings", "start");
      await updateDoc(endDocRef, { start: new Date(dateStart) });
      setMessage({ text: "تم تحديث تاريخ البداية بنجاح!", type: "success" });
    } catch (error) {
      console.error("Error updating end date:", error);
      setMessage({ text: "حدث خطأ أثناء تحديث تاريخ البداية.", type: "error" });
    }
  };

  return (
      <div className="bg-white p-6 rounded w-96 text-right">
        <h1 className="text-2xl font-bold mb-4 text-center"> تاريخ بدايةالتصويت</h1>
